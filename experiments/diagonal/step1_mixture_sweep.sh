...
#SBATCH --array=0-107
#SBATCH --output=logs/step1_mixture_%A_%a.out
#SBATCH --error=logs/step1_mixture_%A_%a.err
...
echo "[SLURM] Step 1 Mixture Sweep"

# Ensure we run from project root and logs exist
cd /home/na658/multi-task2
mkdir -p logs
mkdir -p results/diagonal/step1_mixture
mkdir -p figures/validation_step1
...
$PY experiments/diagonal/step1_mixture_sweep.py $SLURM_ARRAY_TASK_ID
