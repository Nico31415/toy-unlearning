...
#
# IMPORTANT:
#   We write to a *different* RUN_NAME / master CSV per LR to avoid overwriting.
#   (The python upsert key does not include lr, and save_folder naming does not include lr.)
#
# Fixed PTFT config:
...
#
# Submit:
#   sbatch experiments/diagonal/slurm/run_empirical_ptft_lr_sweep_check_gamma1_splitcsv.sh
#
...
# Build a stable tag like lr0p50, lr0p20, lr0p10, lr0p05
LR_FMT="$(printf '%.2f' "$LR")"
LR_TAG="lr${LR_FMT/./p}"

echo "[SLURM] LR=${LR_FMT} (${LR_TAG}) SEED=${SEED}"

ALPHAS_JSON='[0.105]'
SEEDS_JSON="[$SEED]"

RUN_NAME="${RUN_NAME_PREFIX:-emp_ptft_lr_sweep_check_gamma1_alpha0105}_${LR_TAG}"
