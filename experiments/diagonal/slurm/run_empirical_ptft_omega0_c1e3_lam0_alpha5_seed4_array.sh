...
# Fixed params: rho_pt=rho_ft=0.10, c_pt=1e-3, gamma_reinit=0.0
# Sweeps:
#   - omega in {0.0, 1.0}
#   - lambda_pt in {-0.00099, 0.0, +0.00099}  (i.e. ±0.99*c_pt and 0)
#   - alphas=linspace(0.01, 0.2, 5)
#   - seeds=[0,1,2,3]
# Total tasks = 2*3*5*4 = 120  => array=0-119
#
# Submit:
#   sbatch experiments/diagonal/slurm/run_empirical_ptft_omega0_c1e3_lam0_alpha5_seed4_array.sh
#

#SBATCH --job-name=emp_ptft_sweep_g0
#SBATCH --time=12:00:00
#SBATCH --cpus-per-task=1
#SBATCH --mem=10G
#SBATCH --ntasks=1
#SBATCH --array=0-119
#SBATCH --output=logs/slurm/emp_ptft_sweep_g0_%A_%a.out
#SBATCH --error=logs/slurm/emp_ptft_sweep_g0_%A_%a.err
#
...
# np.linspace(0.01, 0.2, 5)
ALPHAS_JSON='[0.01,0.0575,0.105,0.1525,0.2]'
SEEDS_JSON='[0,1,2,3]'

RUN_NAME="${RUN_NAME:-emp_ptft_sweep_c1e3_gamma0_alpha5_seed4}"
