...
"""
EXPERIMENT 1 (REPLICA / THEORY):
DO WE BENEFIT FROM EXISTING FEATURES?

Mirror the parameter combinations used by compute_emp_curves_worker_exp1.py, but
...
def main() -> None:
    ...
    omegas = [0.0, 1.0]  # baseline 0.5 excluded
    ...
    # 0) baseline
    param_combinations += [
        ("baseline", rho_pt, rho_ft, OMEGA_BASE, C_BASE, LAMBDA_BASE, GAMMA_BASE, seed)
    ]

    # 1) vary omega
    param_combinations += [
        ("sweep_omega", rho_pt, rho_ft, omega, C_BASE, LAMBDA_BASE, GAMMA_BASE, seed)
        for omega in omegas
    ]

    # 2) vary c_pt
    param_combinations += [
        ("sweep_c", rho_pt, rho_ft, OMEGA_BASE, c_pt, LAMBDA_BASE, GAMMA_BASE, seed)
        for c_pt in cs
    ]

    # 3) vary lambda_pt
    param_combinations += [
        ("sweep_lambda", rho_pt, rho_ft, OMEGA_BASE, C_BASE, lambda_pt, GAMMA_BASE, seed)
        for lambda_pt in lambda_pts
    ]

    # 4) vary gamma_reinit
    param_combinations += [
        ("sweep_gamma", rho_pt, rho_ft, OMEGA_BASE, C_BASE, LAMBDA_BASE, gamma_reinit, seed)
        for gamma_reinit in gamma_reinits
    ]
