...
#SBATCH --output=logs/emp_corr_q_%A_%a.out
#SBATCH --error=logs/emp_corr_q_%A_%a.err

set -euo pipefail

REPO_ROOT="/home/na658/multi-task2"
OUT_DIR="$REPO_ROOT/results/sanity_check_correlated_overlap_q_sweep"
...
echo "[SLURM] Correlated overlap q sweep; Regime IV lambda_pt=-0.99*c_pt"
...
"$PY" experiments/diagonal/replica/compute_emp_correlated_overlap_worker.py \
  --task-id "$task_id" \
  --omega 0.5 \
  --output-dir "$OUT_DIR"
