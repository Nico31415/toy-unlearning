...
"""
EXPERIMENT 3 (REPLICA / THEORY):
CAN WE GET INTO THE NESTED FEATURE REGIME?

Mirror the parameter combinations used by compute_emp_curves_worker_exp3.py, but
run the replica fixed-point solver and compute an alpha curve per task.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Single seed for replica runs (default: 0).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="results/replica_ptft_parallel_exp3",
        help="Directory to save results",
    )
    args = parser.parse_args()

    # Fixed / required settings (match empirical Exp3)
    rho_pt = 0.10
    rho_fts = [0.01, 0.04]
    omegas = [1.0, 0.0]
    ...
    print(f"Total param combos: {len(param_combinations)}")
    print(f"Total tasks: {total_tasks}")
    ...
    filename = (
        f"EXPERIMENT3_REPLICA_{sweep_name}"
        f"_omega{omega}"
        f"_rhoft{rho_ft}"
        f"_c{c_pt}"
        f"_lambda{lambda_pt}"
