#!/usr/bin/env python3
"""
Run experiments for γ=1 with varying λ_pt.
We need: γ=1, λ ∈ {-0.00095, 0, +0.00095}, c=0.001

For each config, we need 5 unique (rho_ft, omega) combinations:
- Left subplot: (0.02, 1), (0.04, 1), (0.1, 1)
- Right subplot: (0.1, 0), (0.1, 0.5), (0.1, 1) -- (0.1, 1) shared
"""
...
RHO_PT = 0.1
C_PT = 0.001
GAMMA_REINIT = 1.0  # All experiments use γ=1
FT_REGULARISER_SCALE = 1e-6
A_PT = 1.0
INP_DIM = 500
N_TEST = 2000

# Lambda values to run (we already have λ=0, but include for completeness)
LAMBDA_PT_VALUES = [-0.00095, 0.0, 0.00095]
...
def generate_all_configs():
    ...
    configs = []
    for lpt in LAMBDA_PT_VALUES:
        for rho_ft, omega in CONFIGS:
            configs.append({
                'rho_pt': RHO_PT,
                'rho_ft': rho_ft,
                'omega': omega,
...
def main():
    ...
    if args.list:
        for i, (cfg, nr, ne) in enumerate(configs_to_run):
            ...
            print(f"{i}: lpt={cfg['lambda_pt']:.6f}, om={cfg['omega']}, rft={cfg['rho_ft']} [{', '.join(status)}]")
        return
    ...
    if args.task_id is not None:
        ...
        print(f"Task {args.task_id}: lpt={cfg['lambda_pt']:.6f}, om={cfg['omega']}, rft={cfg['rho_ft']}")
        ...
        for i, (cfg, needs_replica, needs_empirical) in enumerate(configs_to_run):
            print(f"\n[{i+1}/{len(configs_to_run)}] lpt={cfg['lambda_pt']:.6f}, om={cfg['omega']}, rft={cfg['rho_ft']}")
