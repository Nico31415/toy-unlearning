#!/usr/bin/env python3
"""
Plot BG alpha sweep results with replica theory overlay.
"""
...
def main():
    ...
    # Extract learning rate from save_folder paths
    lr_str = '5.0'  # default
    if 'save_folder' in df.columns and len(df) > 0:
        lrs = df['save_folder'].str.extract(r'lr=([\d.]+)')[0].dropna().unique()
        if len(lrs) > 0:
            lr_str = str(lrs[0])
    ax.set_title(
        f'BG Alpha Sweep Results (lr={lr_str}, threshold=1e-12)\n'
        f'$\\rho={rho:.3f}$, $c={c:.3f}$, successes = {n_success}/{len(df)}',
        fontsize=14
    )
    ...
    png_path = os.path.join(output_dir, 'bg_alpha_sweep_results.png')
    pdf_path = os.path.join(output_dir, 'bg_alpha_sweep_results.pdf')
